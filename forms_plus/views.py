...
from .models import Form, FormEntry, FieldEntry

fs = FileSystemStorage(location=settings.FORMS_UPLOAD_ROOT)


def entries_list_view(request):
    template = 'admin/forms_plus/entries_list.html'
    context = {'form_pages': Form.objects.published()}
...
def entries_view(request, form_id):
    ...
    template = "admin/forms_plus/entries.html"
    context = {
